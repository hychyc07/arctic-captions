...
class LSTM_att(object):
    def __init__(self, nh_enc, nh_dec, nh_att, nx, ny, mb, lt, bidir, nonlstm_encode=False, restriction=None):
        ...
        a= np.zeros((mb, ny), dtype=config.floatX)
        a[:,0] =1
        ...
        y_din = T.tensor3()
        y_decinput = y_din.astype(config.floatX)   # decode input labels, shifted to right by one and start with eos
        ...
        self.classify = theano.function(inputs=[x_in, y_din], outputs=y_pred)
        self.sentence_train = theano.function(inputs=[x_in, y_din, y_target, lr],
        ...
        self.f_grad_shared, self.f_update = self.optm(lr, dict(zip([s.name for s in self.params], self.params)),
                                                      sentence_gradients, x, y_din, y_target, sentence_cost)

    def train(self, x, y_decinput,  y, learning_rate):
        #for (x_batch, y_batch) in train_batches:
            # here x_batch and y_batch are elements of train_batches and
            # therefore numpy arrays; function MSGD also updates the params
        #    print('Current loss is ', self.sentence_train(x_batch, y_batch, learning_rate))
        cost = self.sentence_train(x, y_decinput, y, learning_rate)
        return cost
        # self.normalize()
