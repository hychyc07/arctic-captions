...
momvalue = np.float32(0.5)
beta = theano.shared(momvalue,'beta')
## to store the prev movements
...
#
print "learning rate"
decreasing_factor = theano.shared(np.float32( (30000.0/batchsize)*10.0 ))
increment = theano.shared(np.float32(1.0))
decreasing_interval = theano.shared(np.float32( (30000.0/batchsize)*10.0 ))
#
...
for pi, (param, gparam, mparam) in enumerate(zip(params, gparams, ms)):
    
    paramchange = beta*mparam + (1-beta)*(-learning_rate * gparam)
    newparam = param + paramchange
    updates.append((param, newparam))
    updates.append((mparam, paramchange))
...
for epi in range(max_epoch):
    ...
    if epi >= 5 and epi < 10:
        beta.set_value(0.9)
        #momvalue = np.float32(0.9)
    elif epi >= 10:
        #momvalue = np.float32(0.95)
        beta.set_value(0.95)
    #else:
    #    momvalue = np.float32(0.5)
    #
    ...
    for step in range(steps):
        ...
        err += train_cost
        print "finish the batch in the epoch", step, epi, beta.get_value()
