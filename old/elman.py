...
class ELMANSLU(object):
    ...
    def __init__(self, nh, nc, nf, mb):
        ...
        idxs = T.itensor3()     # input, since batched, dim rise to 3
        x = idxs.astype(theano.config.floatX)
        yinput = T.itensor3()   # labels
        ...
        self.optm = optimizer.sgd
        self.f_grad_shared, self.f_update = self.optm(lr, self.params, sentence_gradients,
                                      x, y_sentence, sentence_nll)
    ...
    def set_optimizer(self, optmname):
        ...
        if optmname == 'adadelta':
            self.optm = optimizer.adadelta
        if optmname == 'rmsprop':
            self.optm = optimizer.rmsprop
        else:
